...
This script fine-tunes the Phi-3-Mini model using LoRA to extract structured metadata
from natural language payment descriptions (reverse of the original model).
"""
...
CONFIG = {
    ...
    'dataset_dir': './reverse_payments_dataset',
...
def load_and_prepare_datasets(tokenizer):
    """Load and tokenize the reverse payments dataset"""
    print("\n" + "="*80)
    print("LOADING REVERSE DATASET")
    print("="*80)
    ...
    def generate_prompt(data_point: Dict) -> str:
        ...
        This is REVERSED from the original model:
...
{data_point['input']}<|end|>
<|assistant|>
{data_point['output']}<|end|>"""
    ...
    train_dicts = [{"input": ex["input"],
                    "output": ex["output"]}
                   for ex in train_data]
    val_dicts = [{"input": ex["input"],
                  "output": ex["output"]}
                 for ex in val_data]
...
def train_model():
    ...
    print("\n" + "="*80)
    print("PHI-3 REVERSE PAYMENTS FINE-TUNING")
    ...
    try:
        ...
        print("  python test_reverse_model.py")
...
def main():
    ...
    if not os.path.exists(CONFIG['dataset_dir']):
        ...
        print("  python generate_reverse_dataset.py")
        return
